...
func experiencePage(w http.ResponseWriter, r *http.Request) {
	...
	templatePage := filepath.Join("templates", "experience.gohtml")
	...
	var editExperience experienceEdit
	// Route check
	if r.URL.Path != "/" {
		http.NotFound(w, r)
		return
	...
	case "POST":
		...
		editExperience = experienceEdit{
			Message: r.FormValue("message"),
			Success: true,
			Feedback: "feedback sent",
		}
		// do something with editExperience.Message etc. above code is just to test
		tmplErr := templates.ExecuteTemplate(w, "layout", editExperience)
		if tmplErr != nil {
			log.Println(tmplErr.Error())
			http.Error(w, http.StatusText(500), http.StatusInternalServerError)
		}
