...
func skillsPage(w http.ResponseWriter, r *http.Request) {
	...
	templatePage := filepath.Join("templates", "skills.gohtml")
	...
	case "POST":
		...
		// do something with editSkills.Message etc. above code is just to test
		tmplErr := templates.ExecuteTemplate(w, "layout", editSkills)
		if tmplErr != nil {
			log.Println(tmplErr.Error())
			http.Error(w, http.StatusText(500), http.StatusInternalServerError)
		}
