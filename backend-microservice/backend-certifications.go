...
type certificationsEdit struct {
	Message  string
	Success  bool
	Feedback string
}
...
func certificationPage(w http.ResponseWriter, r *http.Request) {
	...
	templatePage := filepath.Join("templates", "certifications.gohtml")
	...
	var editCertifications certificationsEdit
	// Route check
	if r.URL.Path != "/" {
		http.NotFound(w, r)
		return
	...
	case "POST":
		...
		editCertifications = certificationsEdit{
			Message: r.FormValue("message"),
			Success: true,
			Feedback: "feedback sent",
		}
		// do something with editCertifications.Message etc. above code is just to test
		tmplErr := templates.ExecuteTemplate(w, "layout", editCertifications)
		if tmplErr != nil {
			log.Println(tmplErr.Error())
			http.Error(w, http.StatusText(500), http.StatusInternalServerError)
		}
