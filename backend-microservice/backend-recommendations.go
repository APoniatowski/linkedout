...
type recommendationsEdit struct {
	Message  string
	Success  bool
	Feedback string
}
...
func recommendationsPage(w http.ResponseWriter, r *http.Request) {
	...
	templatePage := filepath.Join("templates", "recommendations.gohtml")
	...
	var editRecommendations recommendationsEdit
	// Route check
	if r.URL.Path != "/" {
		http.NotFound(w, r)
		return
	...
	case "POST":
		...
		editRecommendations = recommendationsEdit{
			Message: r.FormValue("message"),
			Success: true,
			Feedback: "feedback sent",
		}
		// do something with editRecommendations.Message etc. above code is just to test
		tmplErr := templates.ExecuteTemplate(w, "layout", editRecommendations)
		if tmplErr != nil {
			log.Println(tmplErr.Error())
			http.Error(w, http.StatusText(500), http.StatusInternalServerError)
		}
